...
int main() {
  ...
  Conds::write_termvec_to_file("/home/hongcez/mingkai/piccolo/s2_s3_asmpt.smt2", global_assumptions);
  return 0;
  ...
  LastState.write_to_file("/home/hongcez/mingkai/piccolo/s2_s3.smt2");
#endif // do not modify anything above, the smt2 is generated by the above process so avoid re-computation

  LastState.read_from_file("/home/hongcez/mingkai/piccolo/s2_s3.smt2");
  auto bw_asmpt = global_assumptions;
  bw_asmpt.push_back( Eq(Sv("s1_to_s2$D_IN"),1) );
  bw_asmpt.push_back( Eq(Sv("s1_to_s2$EN"),1) );
#if 0
  auto s1_to_s2_State = LastState.backward(bw_asmpt);
  // approximately, mkCPU.v 3223 line
  // failure sygus rewriting failure
  // cannot remove input14841  which is removable ...
  /*ERROR:
    SyGuS rewriting failed!
    terminate called after throwing an instance of 'SimulatorException'
    what():  Cannot eliminate var: input14841
  */
  //   TODO:  new strategy, when var is multi-bit and only appear as (extract ...)
  //          do it bit-by-bit, and if after replacing one bit, the two are equivalent...
  //                                no need to branch ...

  s1_to_s2_State.print();
  s1_to_s2_State.write_to_file("/home/hongcez/mingkai/piccolo/s1_s2.smt2");
#endif
  Conds s1_to_s2_State(sts);
  s1_to_s2_State.read_from_file("/home/hongcez/mingkai/piccolo/s1_s2.smt2");

  auto stall_asmpt = global_assumptions;
  stall_asmpt.push_back( OR(Eq(Sv("s1_to_s2$D_IN"),0), Eq(Sv("s1_to_s2$EN"),0)) );

  auto s1_to_s2_State_fwd = TransCheck(s1_to_s2_State, bw_asmpt, LastState, NULL);
  auto s1_to_s2_State_stall = TransCheck(s1_to_s2_State, stall_asmpt, s1_to_s2_State, NULL);
  std::cout << "fwd check: " << s1_to_s2_State_fwd << std::endl; // true
  std::cout << "stall check: " << s1_to_s2_State_stall << std::endl; // false
  // TODO: need EnvInv !!!
  ...
  // HZ: TODO: next check s1_to_s2_State ----bw_asmpt---> LastState     (proceed)
  //                      s1_to_s2_State ----Stall---> s1_to_s2_State   (stalled)
